...
import { createDoor } from '../../actions/customProducts/doorActions'
import { DOOR_CREATE_RESET } from '../../constants/customProductConstants/doorConstants'

const DoorCreateScreen = ({ history }) => {
  ...
  const doorCreate = useSelector(state => state.doorCreate)
  const { loading, error, success } = doorCreate
  ...
  useEffect(() => {
    ...
    if (success) {
      dispatch({ type: DOOR_CREATE_RESET })
  ...
  return (
    <>
      ...
      <FormContainer>
        <h1 className='text-center'>Create Door</h1>
        {loading ? (
          <Loader />
        ) : error ? (
...
export default DoorCreateScreen
